...
export type StationStatusProps = {
  wsUrl: string,
  idToken: string,
  station: string
}
...
export default function StationStatusContent(props: StationStatusProps) {
  ...
  const [stationDetail, setStationDetail] = useState<StationDetailProps>();
   ...
   */
  const updateStationDetail = (message: StationDetailProps) => {
    //console.log('message:', message)
    if (message && message.station) {
      setStationDetail(message);
      setShowProgressBar(false);
    }
    else if(message && message.station == null)
      setShowProgressBar(false);
  };
   ...
   */
  useEffect(() => {
    if (stationDetail == null && lastMessage !== null && lastMessage.data.length > 0) {
      const message = JSON.parse(lastMessage.data);
        if (message.type === 'station_detail') {
          updateStationDetail(message);
        }
    }
  ...
  useEffect(() => {
    ...
    if (lastMessage == null || lastMessage.data.length == 0) {
      if(stationDetail == null && storedBedrockSessionId) {
        onGetStationDetail(storedBedrockSessionId, stationId)
      }
    }
  }, [lastMessage, sendMessage]);
   ...
   */
  const onGetStationDetail = async (sessionId: string, message: string) => {
    const data = {
      type: 'station_detail',
      station: message,
    ...
    <Container
        ...
        <ColumnLayout columns={2}>
          <div>
            ...
            <div>{stationDetail?.station.address}</div>
          </div>
          <div>
            <Box variant="awsui-key-label">Fuel Pumps</Box>
            <div>{stationDetail?.station.fuelPumps}</div>
          </div>
          <div>
            <Box variant="awsui-key-label">Parking Spaces</Box>
            <div>{stationDetail?.station.parkingSpaces}</div>
          </div>
          <div>
            <Box variant="awsui-key-label">Hours of Operation</Box>
            <div>{stationDetail?.station.hoursOfOperation}</div>
          </div>
          <div>
            <Box variant="awsui-key-label">Access to Major Roads</Box>
            <div>{stationDetail?.station.accessToMajorRoads ? "Yes" : "No"}</div>
          </div>
          <div>
            <Box variant="awsui-key-label">Status</Box>
            <StatusIndicator type={stationDetail?.station.state == "In Service" ? "success" : stationDetail?.station.state == "Maintenance" ? "warning" : "error"}>{stationDetail?.station.state}</StatusIndicator>
          </div>
        </ColumnLayout>
      </Container>
  );
